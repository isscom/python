...
array_N = [int(i) for i in input(f'Введите {N} элементов массива через пробел:\n').split()][:N] # ровно то кол-во, кот. скажет пользователь (через пробел)
...
array_M = [int(i) for i in input(f'Введите {M} элементов массива через пробел:\n').split()][:M] #  (через пробел)
# array_M = [int(input(f'{i+1}/{M} элемент: ')) for i in range(M)] # ввод каждого элемента с новой строки
...
print('\n', 'Вывод (результат вычитания множеств):', '\n', *(array_X - array_Y))
# учитывая повторяющиеся элементы в первом множестве - это решение (с set()) не подходит для этой задачи,
# но имеет место быть в других задачах (просто, чтобы помнить о функциях работы со множествами (элементы уникальны): вычитание, сложение)