...
num = int(input('Ввод:\n'))
array_45 = list()
for i in range(num): # проходимся циклом по всем числам от нуля до заданного числа (органичения)
    sum_friendly_numbers = 0 # считаем сумму делителей и при следующей i обнуляем её
    for j in range(1, i // 2 + 1):
        if i % j == 0:
            sum_friendly_numbers += j
    array_45.append((i, sum_friendly_numbers)) # добавляем в список - кортежи (число, делители)
